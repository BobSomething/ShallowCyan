...
array_moves pawn_t::legal_moves(){
    ...
    //promotion
    if(x+d == last_row) {
        arr.push_back(new move_t(location,makep(x+d,y),30)); //queen promotion
        arr.push_back(new move_t(location,makep(x+d,y),31)); //rook promotion
        arr.push_back(new move_t(location,makep(x+d,y),32)); //bishop promotion
        arr.push_back(new move_t(location,makep(x+d,y),33)); //knight promotion
    }
    ...
    if(x == last_row-d) {
        if(check_cell(x,y+1, color, board) == 2)
            if(board->state[x][y+1]->id == "p" && board->state[x][y+1]->color != color && board->last_move->type_move == 22 && board->last_move->after.j == y+1)
                arr.push_back(new move_t(location,makep(x+d,y+1), -2)); //en passant
        if(check_cell(x,y-1, color, board) == 2)
            if(board->state[x][y-1]->id == "p" && board->state[x][y-1]->color != color && board->last_move->type_move == 22 && board->last_move->after.j == y-1)
                arr.push_back(new move_t(location,makep(x+d,y-1), -2)); //en passant
    }
    ...
    if(check_cell(x+d,y, color, board) == 1){ //if next square is empty
        arr.push_back(new move_t(location, makep(x+d,y),21)); //normal move

        if (x == first_time) //first move: you can move two if square is empty
            if(check_cell(x+2*d,y, color, board) == 1)
                arr.push_back(new move_t(location, makep(x+2*d,y),22)); //normal move
    }
    
    //checking if the diagonal captures are legal
    
    if(check_cell(x+d,y+1, color, board) == 2) 
        arr.push_back(new move_t(location, makep(x+d,y+1),-1)); //capture
    else if(check_cell(x+d,y+1, color, board) != 0)
        arr.push_back(new move_t(location, makep(x+d,y+1),-100)); //attack on the square
        ...
        for(auto &[key, value]: directions) {
            if(value == true) {
                ...
                //lands on a cell with the same color piece
                    ...
                    arr.push_back(new move_t(location, makep(x+it*key.i,y+it*key.j), -100)); //on the same color
                    directions[key] = false;
                }

                //if it lands on a cell with a opposite color piece
                else if(check_cell(x+it*key.i,y+it*key.j, color, board) == 2) {
                    arr.push_back(new move_t(location, makep(x+it*key.i,y+it*key.j), -1)); //capture
                    directions[key] = false;
                }
                //if it lands on a empty cell
                else
                    arr.push_back(new move_t(location, makep(x+it*key.i,y+it*key.j), 0)); //normal move
            }
        }
        if(is_going == false)
            break;
...
array_moves knight_t::legal_moves() {
    ...
    for(auto& [i, j]: directions) {
        if(check_cell(x+i,y+j, color, board) == 1) 
            arr.push_back(new move_t(location, makep(x+i,y+j), 0)); //normal move
        else if(check_cell(x+i,y+j, color, board) == 2)
            arr.push_back(new move_t(location, makep(x+i,y+j), -1)); //capture
        else if(check_cell(x+i,y+j, color, board) == 3)
            arr.push_back(new move_t(location, makep(x+i,y+j), -100)); //on the same color
    ...
    for(auto d: directions) {
        ...
        if(check_cell(x+d.i,y+d.j, color, board) == 1) {
            if(board->grid_attack[color][x+d.i][y+d.j] == 0)
                arr.push_back(new move_t(location,makep(x+d.i,y+d.j),0));
        }
        else if(check_cell(x+d.i,y+d.j, color, board) == 2) {
            if(board->grid_attack[color][x+d.i][y+d.j] == 0)
                arr.push_back(new move_t(location,makep(x+d.i,y+d.j),-1));
        }

        else if(check_cell(x+d.i,y+d.j, color, board) == 3) {
            arr.push_back(new move_t(location,makep(x+d.i,y+d.j),-100)); //on the same color 
        }    
    }
        ...
        if(board->state[x][y+1] == nullptr && board->state[x][y+2] == nullptr && board->state[x][y+3] != nullptr) {
            if(board->state[x][y+3]->id == "r" && !board->state[x][y+3]->moved) {
                if(board->grid_attack[color][x][y+1] == 0 && board->grid_attack[color][x][y+2] == 0)
                    arr.push_back(new move_t(location, makep(x,y+2), 1)); //castling
            }
        }
